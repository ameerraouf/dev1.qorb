    ...
    <div class="padding">
        <div class="box">
            <div class="box-header dker">
                <h3>{{ __('cruds.MainServices.Title') }}</h3>
                <small>
                    <a href="{{ route('adminHome') }}">{{ __('backend.home') }}</a> /
                    <a >{{ __('cruds.MainServices.Title') }}</a>
                </small>
            </div>
            @if($main_services->total() > 0)
                @if(@Auth::user()->permissionsGroup->webmaster_status)
                    <div class="row p-a pull-right" style="margin-top: -70px;">
                        <div class="col-sm-12">
                            <a class="btn btn-fw primary" href="{{route("MainServicesCreate")}}">
                                <i class="material-icons">&#xe7fe;</i>
                                &nbsp; {{ __('cruds.MainServices.NewMainService') }}
                            </a>
                        </div>
                    </div>
                @endif
            ...
            @if($main_services->total() > 0)
                {{Form::open(['route'=>'MainServicesUpdateAll','method'=>'post'])}}
                        ...
                        @foreach($main_services as $main_service)
                            <tr>
                                <td class="dker"><label class="ui-check m-a-0">
                                        <input type="checkbox" name="ids[]" value="{{ $main_service->id }}"><i
                                            class="dark-white"></i>
                                        {!! Form::hidden('row_ids[]',$main_service->id, array('class' => 'form-control row_no')) !!}
                                    </label>
                                </td>
                                ...
                                <td class="text-center">
                                    <a class="btn btn-sm success"
                                       href="{{ route("MainServicesEdit",["id"=>$main_service->id]) }}">
                                        <small><i class="material-icons">&#xe3c9;</i> {{ __('backend.edit') }}
                                        </small>
                                    </a>
                                    @if(@Auth::user()->permissionsGroup->webmaster_status)
                                        <button class="btn btn-sm warning" data-toggle="modal"
                                                data-target="#delete-{{ $main_service->id }}" ui-toggle-class="bounce"
                                                ui-target="#animate">
                            ...
                            <!-- .modal -->
                            <div id="delete-{{ $main_service->id }}" class="modal fade" data-backdrop="true">
                                <div class="modal-dialog" id="animate">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <a href="{{ route("MainServicesDestroy",["id"=>$main_service->id]) }}"
                ...
                <footer class="dker p-a">
                    <div class="row">
                        ...
                        <div class="col-sm-3 text-center">
                            <small class="text-muted inline m-t-sm m-b-sm">{{ __('backend.showing') }} {{ $main_services->firstItem() }}
                                -{{ $main_services->lastItem() }} {{ __('backend.of') }}
                                <strong>{{ $main_services->total()  }}</strong> {{ __('backend.records') }}</small>
                        </div>
