...
<script type="text/javascript">
    ...
    var channel = pusher.subscribe('qorb-channel');
    channel.bind('sessions-report-created', function(data) {
...
<script type="text/javascript">
    ...
    var channel = pusher.subscribe('qorb-channel');
    channel.bind('status-report-created', function(data) {
        if (data.data[1] == {{ auth()->user()->id }}) {
            toastr.success(data.data[0]);
        }
    });
...
<script type="text/javascript">
    ...
    var channel = pusher.subscribe('qorb-channel');
    channel.bind('consulting-report-created', function(data) {
        if (data.data[1] == {{ auth()->user()->id }}) {
            toastr.success(data.data[0]);
        }
    });
